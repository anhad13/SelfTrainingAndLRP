...
def main(path, supervision_limit=-1, supervised_model=False, vocabulary=None, pickled_file_path=None, bagging=False, semisupervised=False, force_binarize=False):
    ...
    valid_data = load_trees(path + 'valid.penn', vocab=train_data[-1], grow_vocab= (vocabulary==None), binarize= force_binarize)
    test_data = load_trees(path + 'test.penn', vocab=train_data[-1], grow_vocab=False, binarize= force_binarize)
