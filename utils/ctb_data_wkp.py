...
def parse_chinese_sents(fname):
    trees = []
    current_sent = ''
    brackets = 0
    for line in open(fname):
        line = line.strip()
        if line == '' or line[0] == '%' or ('<' in line and '>' in line):
            continue
        for char in line:
            if char == '(':
                brackets += 1
        ...
        if brackets == 0:
            if False:
                ...
                for p in current_sent.split(' '):
                    if len(p.split(')')[0]) > 0 and not '(' in p.split(')')[0]:
                        new_sent.append('chinese_' + p)
                    else:
                        new_sent.append(p)
...
def load_trees(path, vocab=None, grow_vocab=True, supervision_limit=-1, supervised_model=False, semisupervised=False, binarize=False):
    ...
    for directory, dirnames, filenames in os.walk(path):
        for file in filenames:
            trees += parse_chinese_sents(path+'/'+file)
    for id in [1]:
        for sent in trees:
...
def main(path, supervision_limit=-1, supervised_model=False, vocabulary=None, pickled_file_path=None, bagging=False, semisupervised=False, force_binarize=False):
    ...
    if pickled_file_path == None:
        train_data = load_trees(path + '29', vocab=vocabulary, grow_vocab= (vocabulary==None), supervision_limit=supervision_limit, supervised_model=supervised_model, semisupervised=semisupervised, binarize=True)
        ...
        if bagging:
            ...
            train_data = load_trees(path + '29', vocab=pickled_training_data[-1], grow_vocab= False, binarize=True)
    ...
    valid_data = load_trees(path + '30', vocab=train_data[-1], grow_vocab= (vocabulary==None), binarize= force_binarize)
    test_data = load_trees(path + '31', vocab=train_data[-1], grow_vocab=False, binarize= force_binarize)
