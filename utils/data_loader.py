...
def get_labelled_brackets(tree, idx=0):
    ...
    if isinstance(tree, list) or isinstance(tree, nltk.Tree):
        for node in tree:
            ...
            if next_idx - idx > 1:
                brackets.add((idx, next_idx, tree.label().split("|")[-1]))
                brackets.update(node_brac)
            idx = next_idx
        return brackets, idx
    else:
...
def get_pred_labelled_bracketed(tree, idx=0):
    brackets = set()
    if isinstance(tree, list):
        if len(tree)==3:
            left, right, head_c = tree
            node_brac, next_idx = get_pred_labelled_bracketed(left, idx)
            if next_idx - idx > 1:
                brackets.add((idx, next_idx, head_c))
                brackets.update(node_brac)
            idx = next_idx
            node_brac, next_idx = get_pred_labelled_bracketed(right, idx)
            if next_idx - idx > 1:
                brackets.add((idx, next_idx, head_c))
                brackets.update(node_brac)
            idx = next_idx
...
def tree2labellist(tree):
    if isinstance(tree, nltk.Tree):
        ...
        c1, l1 = tree2labellist(tree[0])
        c2, l2 = tree2labellist(tree[1])
        return c1 + current + c2, l1 + l2
    return [], []
...
def build_tree_labelled(depth, sen, labels, label_map):
    assert len(depth) == len(sen) - 1
    if len(sen)==1 or len(sen) == 2:
        if len(labels) > 0:
            return [sen[0],sen[1],label_map[int(labels[0].data)]]
        else:
            return sen
    parse_tree=[]
    idx_max = numpy.argmax(depth)
    curr_label = label_map[int(labels[idx_max].data)]
    if len(sen[:idx_max+1]) >= 1:
        parse_tree.append(build_tree_labelled(depth[:idx_max],sen[:idx_max+1],labels[:idx_max], label_map))
    if len(sen[idx_max+1:]) >= 1:
        parse_tree.append(build_tree_labelled(depth[idx_max+1:],sen[idx_max+1:],labels[idx_max+1:], label_map))
    parse_tree.append(curr_label)
    return parse_tree
...
def load_trees(ids, vocab=None, grow_vocab=True, supervision_limit=-1, supervised_model=False, semisupervised=False, binarize=False, label_vocab = {}):
    ...
    all_sents, all_trees, all_dists, all_brackets, all_words, all_gates, skip_sup, all_labels, all_labelled_brackets, all_label_leafs, all_original_trees = [], [], [], [], [], [], [], [], [], [], []
    counter = 0
    for id in ids:
        ...
        for sent in ptb.parsed_sents():
            ...
            label_list, leaf_list = tree2labellist(sent)
            label_list_ids = []
            leaf_list_ids = []
            for x in label_list:
    ...
    return all_sents, all_dists, all_trees, all_brackets, all_words, all_gates, skip_sup, all_labels, all_labelled_brackets, all_label_leafs, all_original_trees, label_vocab, vocab
